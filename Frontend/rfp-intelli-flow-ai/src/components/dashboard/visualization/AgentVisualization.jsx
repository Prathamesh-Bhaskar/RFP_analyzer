...
import { FileCheck, CheckCircle, AlertTriangle, ClipboardList, Gavel, BarChart, FileText, Sparkles, Brain } from 'lucide-react';
import { Progress } from "@/components/ui/progress";
import AgentConnectionLines from './AgentConnectionLines';
import ThinkingDots from './ThinkingDots';
import AgentProgressBadge from './AgentProgressBadge';

const AgentVisualization = ({ isAnalyzing = false, sessionData = null, onAnalysisComplete = () => {} }) => {
  ...
  const agents = [
    {
      ...
      description: "Analyzing compliance with all requirements and eligibility criteria",
  ...
  useEffect(() => {
    ...
    setAgentStatus(agents.map(agent => ({ id: agent.id, status: "pending", message: "" })));
    ...
    const simulateAgentProgress = (agentIndex) => {
      ...
      setAgentStatus(prev => 
        prev.map((agent, idx) => 
          idx === agentIndex 
            ? { ...agent, status: "in_progress", message: "Starting analysis..." } 
            : agent
        )
      );
  ...
  if (!isAnalyzing && !processComplete) {
    return null;
  }
              ...
              className={`mb-4 p-4 rounded-lg border relative ${
                currentAgent === index && !processComplete 
                  ...
                  : status === "complete" 
                    ? "border-green-200"
                    : "border-gray-200"
              } ${currentAgent === index && !processComplete ? agent.bgColor : status === "complete" ? "bg-green-50" : "bg-gray-50"}`}
            >
                    ...
                    <div className="flex items-center">
                      {status === "in_progress" && (
                        <span className="inline-block mr-2">
                          <Sparkles className={`h-3 w-3 ${agent.color} animate-pulse`} />
                        </span>
                      )}
                      <span className="text-xs text-gray-500">
                        {status === "in_progress" ? message : "Analysis complete"}
                      </span>
                    </div>
                    <span className="text-xs font-medium">
                      {status === "complete" ? "100%" : `${Math.round(currentAgentProgress)}%`}
                    </span>
                  </div>
                  <Progress 
                    value={status === "complete" ? 100 : currentAgentProgress} 
                    className={`h-1.5 ${status === "in_progress" ? "animate-pulse" : ""}`}
                  />
                </div>
              )}
            </div>

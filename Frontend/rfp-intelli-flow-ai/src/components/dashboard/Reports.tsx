...
const AnalysisStep = ({
  ...
  return (
    <div
      className={`bg-white rounded-lg border ${borderColor} shadow-sm mb-6 overflow-hidden`}
    >
...
const Reports: React.FC = () => {
  ...
  // Add this state for tracking analysis
  ...
  async function processRfpData(rfpData: any) {
    try {
      ...
      const ai = new GoogleGenAI({
        apiKey: "AIzaSyA6M-6Ad8ZSIfDN0X5uuTMhNCz6Nr86P3U",
      });
      const prompt = `
  ...
  `;
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            "Unable to parse the extracted data from Gemini API response"
          );
        }
      }
    } catch (error) {
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          // End both loading states
          setIsAnalyzing(false);
          setIsLoading(false);
        }
      }, 1000); // Optional delay to ensure animation is visible
    } catch (error) {
      console.log(error);
      setIsAnalyzing(false);
      setIsLoading(false);
  ...
  const handleAnalysisComplete = () => {
    ...
    setIsAnalyzing(false);
    // Keep isLoading true if we're still waiting for API response
  };
  ...
  // Modify the empty state render to include AgentVisualization
  if (!report) {
    return (
      <div className="flex flex-col h-screen bg-gray-50 items-center justify-center">
        {isAnalyzing ? (
          // Show the analysis visualization when analyzing
          ...
          // Show loading spinner after animation completes but still waiting for API
          ...
          // Show the initial load button
          <div>
            <Button onClick={handleSubmit}>Load Report</Button>
            <p className="mt-4 text-gray-600">
              Click the button to load the RFP data.
            </p>
          </div>
        )}
      </div>
  ...
  const nonCompliantItems = report.compliance.checks.filter(
    (c) => c.status === "not-met" && c.critical
  );
    ...
    <div className="flex h-screen bg-gray-50">
      <div className="flex-1 flex flex-col overflow-hidden">
        <main className="flex-1 overflow-auto p-6" ref={reportRef}>
          <button onClick={handleSubmit}>Reload Report</button>
                ...
                {report.compliance.checks.map((check) => (
                  <div
                    ...
                    className={`flex items-start gap-3 p-3 rounded-lg ${
                      check.status === "not-met" && check.critical
                        ? "bg-red-50"
              ...
              {!isCompliant && (
                <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                  ...
                  <p className="text-sm mt-2">
                    Resolve the critical compliance issues before proceeding
                    with the RFP analysis.
                  </p>
                  <ul className="mt-2 space-y-1">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {report.eligibility.criteria.map((criteria) => (
                  <div
                    key={criteria.id}
                    className={`flex items-start gap-3 p-3 rounded-lg ${
                      criteria.status === "not-met" && criteria.critical
                        ? "bg-red-50"
              ...
              {!report.eligibility.proceed && (
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                  ...
                  <p className="text-sm mt-2">
                    While not mandatory, meeting these eligibility criteria
          ...
          <AnalysisStep
            ...
            status={
              report.checklist.items.some((i) => i.status === "pending")
                ? "warning"
                : "success"
            }
          ...
          <AnalysisStep
            ...
            status={
              report.risks.some((r) => r.riskLevel === "high")
                ? "warning"
                : "success"
            }
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                <div className="p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                  ...
                  <p className="text-2xl font-semibold">
                    {
                      report.risks.filter((r) => r.riskLevel === "medium")
                        .length
                    }
                  </p>
                </div>
              ...
              <div className="p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
                ...
                <ol className="mt-2 space-y-3 list-decimal list-inside">
                  ...
                  <li className="text-sm pl-2">
                    Submit complete proposal package by{" "}
                    {new Date(report.submissionDeadline).toLocaleDateString()}
                  </li>
                </ol>
              </div>
